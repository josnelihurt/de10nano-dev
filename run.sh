...
#those files but I don't have enoght time for that at this moment
./downloader/run.sh

DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
cd $DIR

#this will build the main builder image, also It will create an http server in your main machine
