...
#include <stdint.h>
#include <sys/mman.h>
//This should be regenerated from $sopc-create-header-files ../../soc_system.sopcinfo --single hps_0.h --module hps_0
...
int main(int argc, char ** argv)
{
    ...
    for(int j = 0; j < 8; ++j){
        *custom_led_map = (1<<(8-j));
        usleep(sleep_time);
    }
    
    printf("changing grid color \n");
    custom_led_map++;
    for(int j=0;j<200;j++)
    for(int i = 0; i < 7; ++i) {
        *custom_led_map = i;
